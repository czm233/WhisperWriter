...
from app_switcher import switch_to_app
from output_er import send_text_to_window
...
global_initial_prompt = "这段音频可能包含技术术语，如 Vue.js JavaScript Node.js Vue React Django Drf Python 队列等编程软件领域的词语术语"
...
def transcribe_audio(model, audio_queue, stop_event, text_queue, language="zh", fp16=False):
    """
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            result = model.transcribe(audio_array,
                                      language=language,
                                      fp16=fp16,
                                      initial_prompt=global_initial_prompt,
                                      temperature=0,
                                      beam_size=10,
                                      best_of=10,
...
def output_text(stop_event, text_queue):
    """输出文本，等待队列收到文本后开始。"""
    print("输出线程已启动。")
    while not stop_event.is_set():
        try:
            ...
            keyword = "切换到"
            if keyword in text:
                app_title = text.split(keyword, 1)[1].strip()
                switch_to_app(app_title)
            else:
                send_text_to_window(text, 0)
        except queue.Empty:
            continue
    print("输出线程已停止。")
...
def main():
    ...
    text_queue = queue.Queue() # 文本队列

    #输出文字线程
    output_text_thread = threading.Thread(
        target=output_text,
        args=(stop_event, text_queue),
        daemon=True
    )
    output_text_thread.start()
    ...
    transcription_thread = threading.Thread(
        ...
        args=(model, audio_queue, stop_event, text_queue),
        daemon=True
    )
    ...
    try:
        ...
        output_text_thread.join(timeout=2)
        p.terminate()
        print("主线程已退出。")
